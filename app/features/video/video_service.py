...
class VideoService:
    ...
    def search_and_analyze_video(self, request: SearchRequest) -> SearchResponse:
        ...
        # 문자/숫자/공백만 남기고 나머지는 전부 공백으로 치환(화이트리스트 방식).
        # 원래는 |&"-() 등 몇 개만 블랙리스트로 지웠는데, 한국 유튜브 제목에 흔한
        # ≪≫《》: 같은 문자를 못 걸러서 YouTube 검색이 완전히 엉뚱한 결과를 반환하는
        # 문제가 있었다(실사용자 재현: "≪메이드 인 코리아≫ 시즌2.. 드디어 떴다.."를
        # 그대로 검색하면 ≪≫가 안 지워져 전혀 다른 영상이 1위로 나오고, 같은 문자열을
        # 90초 간격으로 4회 반복해도 결과가 동일함을 확인함 — 즉 검색 결과 자체의
        # 실시간 변동이 아니라 이 특수문자 잔존이 원인이었다).
